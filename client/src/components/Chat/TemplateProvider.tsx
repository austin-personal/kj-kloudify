...
const templates: Record<number, Template> = {
    ...
    13: {
        ...
        buttons: [
            { id: 6, label: "비용 최적화: 비용을 낮추고 저용량부터 시작할 수 있는 설정 (예: 작은 RDS 인스턴스, 온디맨드 가격 모델)" },
            { id: 7, label: "고성능: 높은 성능과 빠른 처리 속도를 위해 최적화된 설정 (예: 고성능 RDS 인스턴스, Provisioned IOPS 스토리지)" },
            { id: 8, label: "확장 가능성: 서비스 확장을 위한 자동 확장 옵션 (예: Aurora Serverless)"}
        ]
    },
    14: {
        name: "서버설정",
        ...
        buttons: [
            { id: 9, label: "저비용 서버: 일반적인 웹 서비스나 소규모 트래픽을 위한 저비용 옵션 (예: 작은 EC2 인스턴스, Spot Instances)" },
            { id: 10, label: "성능 중심 서버: 트래픽이 많거나 성능이 중요한 경우 (예: 고성능 EC2 인스턴스, Enhanced Networking 지원)" },
            { id: 11, label: "서버리스: 관리가 필요 없는 자동 확장 서버리스 옵션 (예: AWS Lambda)"}
        ]
    },
    15: {
        name: "스토리지설정",
        ...
        buttons: [
            { id: 12, label: "비용 절감: 장기 저장 및 저렴한 비용이 필요할 때 (예: S3 Standard-IA, S3 Glacier)" },
            { id: 13, label: "고성능: 빈번한 데이터 접근을 위한 높은 성능 (예: S3 Standard)" },
            { id: 14, label: "확장 및 내구성: 자동 확장 및 높은 데이터 내구성을 원하는 경우 (예: S3와 자동 확장 설정)"}
        ]
    },
    16: {
        name: "네트워크설정",
        ...
        buttons: [
            { id: 15, label: "기본 보안: 기본적인 보안 구성으로 클라우드 네트워크 보호" },
            { id: 16, label: "고급 보안: 보안 강화를 위한 VPN 연결 및 세분화된 접근 제어 (예: VPC, Network ACL)" },
            { id: 17, label: "성능 최적화: 네트워크 성능을 높이기 위한 고성능 설정 (예: 고성능 네트워킹, 글로벌 가속기)"}
        ]
    },
    17: {
        name: "모니터링설정",
        ...
        buttons: [
            { id: 18, label: "기본 모니터링: 기본적인 성능 모니터링과 에러 알림 (예: CloudWatch 기본 설정)" },
            { id: 19, label: "심화 모니터링: 더 상세한 성능 및 로그 데이터 수집 (예: CloudWatch와 고급 메트릭)" },
