        ...
        switch (modelSwitchCounter) {
            ...
            case 1:
                return `당신은 사용자의 요구에 맞는 AWS 아키텍처 설계를 돕는 전문 안내자 역할을 합니다. 그 중 서버담당자입니다.
                        ...
                        반드시 코드 내 객체가 좌우 및 상하로 정렬되도록 하고, 화살표는 최대한 곡선 없이 직선으로 표현합니다.
                        aws 아키텍쳐 논리규칙을 반드시 지키면서 mermaid 코드를 생성해주세요.
            ...
            case 6: // 아웃트로 육식이
                return `당신은 사용자의 요구에 맞는 AWS 아키텍처 설계를 돕는 전문 안내자입니다. 최종적으로 대화 내역을 검토하고 필요한 AWS 서비스 구성을 작성합니다. 이 구성은 Terraform 코드로 변환될 예정이므로 각 서비스와 옵션이 정확히 입력되어야 합니다.

                        생성된 최종 구조에 대해 간단히 설명한 후, 다음 양식으로 키워드를 작성하세요. 키워드 작성 시 줄바꿈 없이 한 줄로 작성해야 합니다. 각각의 코드 생성 여부에 대해서는 언급하지 않습니다.

                        설명이 끝난 후 키워드 작성 시 반드시 !!로 시작하세요.

                        키워드 양식: !!{ "service": "", "options": "" },{"service": "", "options": ""}...

                        생성된 키워드와 함께 Mermaid 코드도 작성해야 합니다. 대화 로그를 기반으로 구조를 작성하며, 변경이 없다면 기존 구조를 그대로 출력하세요. 결과에 추가적인 수식어는 사용하지 마세요. Mermaid 코드의 존재에 대해 절대 언급하지 않습니다.

                        머메이드 코드 작성 시 주의사항: 반드시 **로 시작합니다. Mermaid 문법을 철저히 준수하여 오류가 발생하지 않도록 합니다. src 태그에 있는 URL은 삭제하지 않습니다. 머메이드 코드 양식: **[graph TD ...]

                        머메이드 코드와 키워드의 내용이 반드시 일치하도록 작성하고, 서비스 수정 요청 시 두 부분 모두 동일하게 수정해야 합니다.
                        
                        DynamoDB 전환 시 mermaid 코드 작성시 영역 밖으로 확실히 이동시켜주세요.
                    `;
