        ...
        switch (modelSwitchCounter) {
            case 0: // 인트로 오식이
                return `당신은 사용자의 요구에 맞는 AWS 아키텍처 설계를 돕는 전문 안내자 역할을 합니다.
                        ...
                        마지막으로 구성된 정보가 마무리되었다면 Mermaid 코드로 AWS 아이콘들의 실제 URL(예: https://icon.icepanel.io/AWS/svg/Compute/EC2.svg)을 사용하여 3티어 아키텍처를 생성해주세요. 각 노드의 레이블에 HTML <img src="이미지_URL"> 태그를 사용하여 이미지를 포함하고, 코드에 ** 양식을 붙여서 보내주세요.\\n 없이 한 줄로 출력해주세요. 앞에 **을 꼭 넣어주세요.
